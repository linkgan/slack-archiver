...
class Archive:
  ...
  def _to_file(self, messages, channel, type)-> None:
    ...
    result = {'channel_name': channel, 'channel_id': self.channel_dict[channel], 'messages': messages}
  ...
  def get_threads(self, channel_id)->dict:
    ...
    try:
      threads = self.df[self.df['thread_ts'].notnull()]['thread_ts'].values.tolist()
    except KeyError:
  ...
  def gsheets_io(self, channel:str):
    ...
    self.gspread.df_to_sheet(self.df, index=False, sheet=sheet_name, start='A3', replace=True)
...
def load_config(filename:str='config.yaml')->dict:
  if os.path.isfile(filename):
    ...
    logger.error("Config file path does not exist, please create a config file before running.")
